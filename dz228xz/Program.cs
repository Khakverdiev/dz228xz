...
//var debtor1 = debtors.Where(v => vowels.Contains(v.FullName[0])).ToList();

//debtor1.ForEach(Console.WriteLine);

//var debtor2 = debtors.Where(e => e.Email.Contains("rhyta.com") || e.Email.Contains("dayrep.com")).ToList();
//debtor2.ForEach(Console.WriteLine);

//DateTime currentDate = DateTime.Now; // чтобы узнать щасошнее время :)

//// а то что -36 и -26 это сколько времени прошло с текущей даты
//var debtor3 = debtors.Where(b => b.BirthDay >= currentDate.AddYears(-36) && b.BirthDay <= currentDate.AddYears(-26)).ToList();

//debtor3.ForEach(Console.WriteLine);

//var debtor4= debtors.Where(s => s.Debt < 5000).ToList();
//debtor4.ForEach(Console.WriteLine);

// IsDigit нужен для того чтобы проверять является ли символ цифрой
//var debtor5 = debtors.Where(l => l.FullName.Length > 18 && l.Phone.Count(char.IsDigit) >= 1 && l.Phone.Count(p => p == '7') >= 2).ToList();

//debtor5.ForEach(Console.WriteLine);

//var debtor6 = debtors.Where(f => f.BirthDay.Year % 2 == 0).OrderBy(f => f.BirthDay.Year).ToList();
//debtor6.ForEach(Console.WriteLine);

// Substring для того чтобы извлечь последние 5 символа из строки адреса 
//var debtor7 = debtors.Where(b => b.BirthDay.Month >= 12 || b.BirthDay.Month <= 2).OrderBy(b => b.Address.Substring(b.Address.Length - 5)).ToList();
//debtor7.ForEach(Console.WriteLine);

//double averageDebt = debtors.Average(debtor => debtor.Debt);
//var debtor8 = debtors.Where(d => d.Debt > averageDebt).OrderBy(d => d.FullName.Split(' ').Last())
//    .OrderByDescending(debtor => debtor.Debt).ToList();

//debtor8.ForEach(Console.WriteLine);

//var debtor9 = debtors.Where(d => !d.Phone.Contains('8'));

//foreach (var result in debtor9)
//{
//    Console.WriteLine($"FullName: {result.FullName.Split(' ').Last()}, Age: {DateTime.Now.Year - result.BirthDay.Year}, Debt: {result.Debt}");
//}

// var debtor10 = debtors.OrderByDescending(debtor => debtor.FullName.Split(' ').Last().Length)
// .OrderBy(debtor => debtor.FullName.Split(' ').First().Length).First();
// Console.WriteLine($"FullName: {debtor10.FullName}, LastName: {debtor10.FullName.Split(' ').Last().Length}");

// var debtor11 = debtors.Where(debtor =>
//     {
//         string fullName = debtor.FullName.Replace(" ", "").ToLower(); 
//         return fullName.Count(c => fullName.Count(ch => ch == c) >= 3) >= 3;
//     })
//     .OrderBy(debtor => debtor.FullName.Split(' ').First()) 
//     .ToList();
//
// if (debtor11.Any())
// {
//     debtor11.ForEach(Console.WriteLine);
// }
// else
// {
//     Console.WriteLine("?");
...
// var debtor12 = debtors
//     .Select(d => d.Email.Split('@').Last())
//     .GroupBy(domain => domain)
//     .OrderByDescending(group => group.Count())
//     .FirstOrDefault();
//
// Console.WriteLine(debtor12.Key);

// var debtor13 = debtors.
//      Select(d => d.BirthDay.Year)
//     .GroupBy(date => date)
//     .OrderByDescending(group => group.Count())
//     .FirstOrDefault();
//
// Console.WriteLine(debtor13.Key);

// var debtor14 = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
//
// if (debtor14.Any())
// {
//     debtor14.ForEach(Console.WriteLine);
// }
// else
// {
//     Console.WriteLine("?");
...
// var debtor15 = debtors.Sum(d => d.Debt);
// Console.WriteLine(debtor15);

// var debtor16 = debtors.Where(d => d.BirthDay.Year <= 1945).ToList();
// debtor16.ForEach(Console.WriteLine);

// var debtor17 = debtors
//     .Where(debtor => debtor.BirthDay.Year > 1991)
//     .OrderBy(d => d.FullName = d.FullName.Split(" ").Last())
//     .OrderBy(d => DateTime.Now.Year - d.BirthDay.Year).ToList();
//
// debtor17.ForEach(Console.WriteLine);

// var debtor18 = debtors.Where(d => d.Phone.Distinct().Count() == d.Phone.Length)
//     .OrderBy(d => d.Phone)
//     .OrderBy(d => d.Debt)
//     .ToList();
//
// debtor18.ForEach(Console.WriteLine);


// var debtor19 = debtors
//     .Where(d => d.BirthDay > DateTime.Now)
//     .Where(d => (d.BirthDay - DateTime.Now).TotalDays > d.Debt / 500)
//     .OrderBy(d => d.FullName)
//     .OrderBy(d => d.BirthDay)
//     .OrderBy(d => d.Debt - (int)(d.BirthDay - DateTime.Now).TotalDays * 500);
//
// if (debtor19.Any())
// {
//     foreach (var debtor in debtor19)
//     {
//         Console.WriteLine($"{debtor.FullName}, Birth Day: {debtor.BirthDay.ToShortDateString()}, Balance owed: {debtor.Debt}");
//     }
// }
// else
// {
...
//var debtor20 = debtors
//    .Where(d =>
//    {
//        string fullName = d.FullName.ToLower();
//        foreach (var letter in word)
//        {
//            if (fullName.Contains(letter))
//            {
//                fullName = fullName.Remove(fullName.IndexOf(letter), 1);
//            }
//            else
//            {
//                return false;
...
//debtor20.ForEach(Console.WriteLine);
